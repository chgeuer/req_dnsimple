...
zone-transfer or reachability requests. Linking sends the required secondary-zone
name in one request without looking up or creating either resource. Deletion
removes only the selected primary-server configuration; it does not unlink zones
or perform DNS requests.
...
| `ReqDnsimple.PrimaryServer` | `/secondary_dns/primaries` | `create/3`, `get/3`, `link/4`, `delete/3`, `from_json/1` |
